...
def admin_registration(request):
    if request.method == 'POST':
        ...
        try:
            ...
            custom_id = generate_admin_id("admin")
...
def t_user_view(request, batch_id):
    ...
    try:
        ...
        corpus = CorpusTable.objects.filter(corpus_id=translator.corpus_id).first()
        language_proficiency = corpus.language_profeciency if corpus else 'N/A'
        target_lang = corpus.target_lang if corpus else 'N/A'
    except TranslatorTable.DoesNotExist:
        return redirect('translator_login')
    ...
    if request.method == 'POST':
        ...
        for row in reader:
            ...
            try:
                ...
                messages.error(request, f'Error saving row: {row}. Error: {e}')
                error_occurred = True
                continue  
        messages.success(request, f'CSV file has been imported successfully! {success_count} entries added.')
